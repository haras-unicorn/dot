...
match $selected.kind {
  "source" => {
    ...
    ui wait $"Sourcing with ($selected.name)..." --on-fail { rm -f $tmp } {
      with-env {
        DOT_TOOLBELT_EXTENSION: ""
        DOT_TOOLBELT_MIME: "none"
      } {
        ^$selected.exe | save -f $tmp
        log "exec" "source done"
      }
    }
  ...
  "node" => {
    ...
    ui wait $"Processing with ($selected.name)..." --on-fail { rm -f $tmp } {
      with-env {
        DOT_TOOLBELT_EXTENSION: $meta.extension
        DOT_TOOLBELT_MIME: $meta.mime
      } {
        open --raw $data_file | ^$selected.exe | save -f $tmp
        log "exec" "node done"
      }
    }
  ...
  "sink" => {
    ...
    ui wait $"Sinking with ($selected.name)..." {
      with-env {
        ...
        open --raw $data_file | ^$selected.exe
        # NOTE: needed stderr for wl-clipboard
        log "exec" "sink done"
      }
    }

...

{
  machines.homeModules.zeroclaw =
    ...
    let
      ...
      # NOTE: the daemon runs under bwrap instead of systemd syscall
      # filtering — bun trips SystemCallFilter (seccomp), while bwrap
      # isolates via namespaces so bun is happy. The shell-tool sandbox
      # (sandbox_backend = "bwrap") needs user namespaces too, which is why
      # RestrictNamespaces is not set on this unit.
      #
      # The network namespace is deliberately shared: the daemon talks to
      # 127.0.0.1 services (CoreDNS on :53, llama.cpp on 8080/8081) and the
      # host network for Matrix. No pid namespace either — systemd manages
      # the cgroup/restart and the daemon reaps its own children.
      start = pkgs.writeShellApplication {
        ...
        text = ''
          exec ${pkgs.bubblewrap}/bin/bwrap \
            ...
            --bind ${nixCache} ${nixCache} \
            --bind ${nixState} ${nixState} \
            --chdir ${dataDir} \
            ${zeroclaw}/bin/zeroclaw daemon
        '';
      };
    ...
    in
    {
      ...
      systemd.user.services.zeroclaw = {
        ...
        Service = {
          ...
          RemoveIPC = true;
          # NOTE: no RestrictNamespaces — bwrap (the outer wrapper and the
          # shell-tool sandbox) needs user + mount namespaces.
          ...
          AmbientCapabilities = [ "" ];
          # NOTE: no SystemCallFilter — bun trips seccomp filters. bwrap does
          # the isolation instead (see the start script).
          UMask = "0077";
          ReadWritePaths = [
            dataDir
            nixCache

...
let
  ...
  optionsAndConfig =
    ...
    let
      ...
      sysfsBusIdToBusId =
        ...
        let
          ...
          bus = flakeLib.hex.hexToDec (builtins.elemAt matched 0);
          dev = flakeLib.hex.hexToDec (builtins.elemAt matched 1);
          fn = flakeLib.hex.hexToDec (builtins.elemAt matched 2);
        in
    ...
    in
    {
      inherit hardwareOptions hardwareConfig;
    };
in
{
  flake.nixosModules.hardware-module =
    { lib, config, ... }:
    let
      result = optionsAndConfig { inherit config lib; };
    in
    {
      options = result.hardwareOptions;
      config = lib.mkMerge [
  ...
  flake.homeModules.hardware-module =
    { config, lib, ... }:
    let
      result = optionsAndConfig { inherit config lib; };
    in

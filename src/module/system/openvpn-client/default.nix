...
in
{
  ...
  config = mkIf cfg.client.enable {
    services.openvpn.servers."${cfg.client.host}".config = ''
      client
      remote ${cfg.client.domain} ${port}
      proto ${protocol}
      dev tun

      ca /etc/openvpn/${cfg.client.host}/root-ca.ssl.crt
      cert /etc/openvpn/${cfg.client.host}/client.ssl.crt
      key /etc/openvpn/${cfg.client.host}/client.ssl.key
      tls-auth /etc/openvpn/${cfg.client.host}/server.ta.key 1
      ...
      verb 3
    '';
    sops.secrets."root-ca.ssl.crt".path = "/etc/openvpn/${cfg.client.host}/root-ca.ssl.crt";
